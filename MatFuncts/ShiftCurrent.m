function [SC_sk,SC_ok,Rnmab_s,Rnmab_o] = ShiftCurrent(o,v,w,r,f,p,wavefunct,orbital_seq,nw,hv,ksi,direct)
nhv = length(hv);
ndir = size(direct,1);
nwl = size(p,3);
SC_sk = zeros(ndir, nhv, nwl);
SC_ok = zeros(ndir, nhv, nwl);
Rnmab_s = zeros(nw,nw,ndir, nwl);  % shift vector
Rnmab_o = zeros(nw,nw,ndir, nwl); 
for idir = 1:ndir
a = direct(idir,1);
b = direct(idir,2);
c = direct(idir,3);
...
for iwl = 1:nwl
Iabcsd = calc_Imn(r,v,o,w,p(:,:,iwl),nw,ksi,a,b,c,sd);
Iacbsd = calc_Imn(r,v,o,w,p(:,:,iwl),nw,ksi,a,c,b,sd);
Iabcod = calc_Imn(r,v,o,w,p(:,:,iwl),nw,ksi,a,b,c,od);
Iacbod = calc_Imn(r,v,o,w,p(:,:,iwl),nw,ksi,a,c,b,od);
