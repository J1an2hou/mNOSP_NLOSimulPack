function [sppump_even,sppump_odd, orbpump_even, orbpump_odd] = SpinOrbPumping_OT(Hxc,o,p,cp,EF,orb_seq,Enk,nw,wf,direct,ksi)
...
% In this expression, we apply the O (S or L) vs torque algorithm, so that we
% denote it as OT in the function name.
...
for icp = 1:ncp
    ...
    for jwl = 1:nwl
        pA = p(:,:,iwl);
        pB = p(:,:,jwl);
        sA = comm(sp_mat, pA, +1)/2;
        sAmn = wf' * sA * wf;
        lA = comm(l_mat, pA, +1)/2;
        lAmn = wf' * lA * wf;
        xctorB = comm(xctor, pB, +1)/2;
        xctorBmn = wf' * xctorB * wf;
        fac_sp = sAmn .* xctorBmn.';
        fac_orb = lAmn .* xctorBmn.';
        % sppump_even(icp, idir, iwl, jwl) = pi*(dfde' * diag(imag(sAmn * xctorBmn)));
        sppump_even(icp, idir, iwl, jwl) = -ksi * trace((f ./ (o.^2+ksi^2)) * imag(fac_sp)) ...
            - sum(real(dfde .* diag(sAmn) .* diag(xctorBmn)));
        sppump_odd(icp, idir, iwl, jwl) ...
            = trace((f.*o./(o.^2+ksi^2)) * real(fac_sp));
        % orbpump_even(icp, idir, iwl, jwl) = pi*(dfde' * diag(imag(lAmn * xctorBmn)));
        orbpump_even(icp, idir, iwl, jwl) = -ksi * trace((f ./ (o.^2+ksi^2)) * imag(fac_orb)) ...
            - sum(real(dfde .* diag(lAmn) .* diag(xctorBmn)));
        orbpump_odd(icp, idir, iwl, jwl) ...
            = trace((f.*o./(o.^2+ksi^2)) * real(fac_orb));
    end
    end
    end
end
