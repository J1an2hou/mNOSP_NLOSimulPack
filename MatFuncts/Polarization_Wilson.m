function Pdir = Polarization_Wilson(wvf_k, nocc, Nk1, Nk2, Nk3, dir)
% ------------------------------------------------------------
...
% Nk1,Nk2,Nk3 : k-mesh sizes (Nk1 is the fastest index)
...
% the lattice vector along the direction dir, and divide it by total volume
    ...
    for p = 1:Nperp
        ...
        for l = 1:Nkdir
            ...
            Uj  = squeeze(wvf_k(k,:,1:nocc));
            Ujp = squeeze(wvf_k(kp,:,1:nocc));
            M = Uj' * Ujp;
            W = M * W;
        end
        Wil_sum = Wil_sum + imag(log(det(W)));
    end
    Pdir = -Wil_sum / (2*pi*Nperp);
...
% For a given longitudinal k index (l) and a perpendicular index (p)
% How to map them into the general k-mesh indices i1 - i3
