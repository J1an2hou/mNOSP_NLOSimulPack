function Hk = Magnus_Hk_SlabZ(R,HR,wtR,wc,nw,kpt,nL,A0,omega,nq,alatt,blatt)
...
[Hkq0,~,~] = buildHk_SurfaceZ(HR.*FloPhase0,wtR,R,wc,nw,kpt,alatt,blatt,nL);
for iq = 1:nq
    FloPhaseq = FloPhase_ave(A0,R,wc,alatt,nw,nR,iq);
    FloPhase_q = FloPhase_ave(A0,R,wc,alatt,nw,nR,-iq);
    [Hkq(:,:,iq),~,~] ...
        = buildHk_SurfaceZ(HR.*FloPhaseq,wtR,R,wc,nw,kpt,alatt,blatt,nL);
    [Hk_q(:,:,iq),~,~] ...
        = buildHk_SurfaceZ(HR.*FloPhase_q,wtR,R,wc,nw,kpt,alatt,blatt,nL);
end
Hk = Hkq0;
for iq = 1:nq
