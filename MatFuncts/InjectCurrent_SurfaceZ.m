function [bopv_on_k,bspv_on_k] ...
    = InjectCurrent_SurfaceZ(v,r,o,f,p,wvf,orbital_seq,nw,nL,hv,direct,ksi)
nhv = length(hv);
ndir = size(direct,1);
nwl = size(p,3);
bopv_on_k = zeros(ndir, nhv,nwl);
bspv_on_k = zeros(ndir, nhv,nwl);
%jorb = zeros(ndir,nwl,nw);
%jspin = zeros(ndir,nwl,nw);
...
for idir = 1:ndir
    ...
    od = calc_orb(nw,nL,wvf,orbital_seq,d);
    sd = calc_sp(nw,nL,wvf,d);
    jod = (od*va+va*od)/2;
    jod = jod + 1i*(ra*(o.*od)+(o.*od)*ra)/2; % This is the added torque term
    jsd = (sd*va+va*sd)/2;
    jsd = jsd + 1i*(ra*(o.*sd)+(o.*sd)*ra)/2; % This is the added torque term
    for iwl = 1:nwl
    jod_proj = jod*p(:,:,iwl); % projection
    jsd_proj = jsd*p(:,:,iwl); % projection
    diffjod = diag(jod_proj)*ones(1,nw*nL) - ones(nw*nL,1)*diag(jod_proj).';
    diffjsd = diag(jsd_proj)*ones(1,nw*nL) - ones(nw*nL,1)*diag(jsd_proj).';
    %jorb(idir,iwl,:) = diag(real(jod_proj));
    %jspin(idir,iwl,:) = diag(real(jsd_proj));
